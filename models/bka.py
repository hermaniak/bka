...
class bka(models.Model):
    _name = "account.bka"
    _inherit = ['mail.thread', 'mail.activity.mixin', 'portal.mixin']
    _description = "Nebenkostenabrechnung"
    _order = 'date_order desc, id desc'

    @api.depends('order_line.price_total','amount_paid')
    def _amount_all(self):
        ...
        for order in self:
            ...
            order.update({
                ...
                'amount_grand_total': amount_untaxed + self.amount_paid,
            })
    ...
    def get_empty_list_help(self, help):
        ...
        return super(bka, self).get_empty_list_help(help)
    ...
    order_line = fields.One2many('account.bka.line', 'order_id', string='Order Lines', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True, auto_join=True)
    ...
    amount_paid = fields.Monetary(string='Vorauszahlung Summe', store=True)
    amount_tax = fields.Monetary(string='Taxes', store=True, readonly=True, compute='_amount_all')
    amount_total = fields.Monetary(string='Zwischensumme', store=True, readonly=True, compute='_amount_all', track_visibility='always')
    amount_grand_total = fields.Monetary(string='Total', store=True, readonly=True, compute='_amount_all', track_visibility='always')
    ...
    def _compute_portal_url(self):
        super(bka, self)._compute_portal_url()
        for order in self:
...
#        return super(bka, self)._track_subtype(init_values)
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            if 'company_id' in vals:
                vals['name'] = self.env['ir.sequence'].with_context(force_company=vals['company_id']).next_by_code('account.bka') or _('New')
            else:
                vals['name'] = self.env['ir.sequence'].next_by_code('account.bka') or _('New')
        ...
        result = super(bka, self).create(vals)
        return result
    ...
    def copy_data(self, default=None):
        ...
        return super(bka, self).copy_data(default)
    ...
    def name_get(self):
        if self._context.get('sale_show_partner_name'):
            ...
            return res
        return super(bka, self).name_get()
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        ...
        return super(bka, self).name_search(name, args, operator, limit)
...
#            super(bka, self)._init_column(column_name)
#        else:
...
#            self.env.cr.execute(query)
    ...
    @api.multi
    def action_print_bka(self):
        self.filtered(lambda s: s.state == 'draft').write({'state': 'sent'})
        return self.env.ref('bka.action_print_bka').report_action(self)
    ...
    def action_invoice_create(self, grouped=False, final=False):
        ...
        for order in self:
                ...
                amount=order.amount_grand_total
                if amount < 0:
                        inv_type='out_refund'
                else:
                        inv_type='out_invoice'
                name='Nachzahlung Nebenkostenabrechnung'
                invoice = inv_obj.create({
                    'name': order.client_order_ref or order.name,
                    'date': fields.Datetime.now(),
                    'date_due': fields.Datetime.now(),
                    'origin': order.name,
                    'reference': 'Nachzahlung Nebenkostenabrechnung', 
                    'type': inv_type,
                    'reference': False,
    ...
    def get_access_action(self, access_uid=None):
        ...
        return super(bka, self).get_access_action(access_uid)
    ...
    def _notification_recipients(self, message, groups):
        groups = super(bka, self)._notification_recipients(message, groups)
...
class BKALine(models.Model):
    _name = 'account.bka.line'
    _description = 'BKA line Line'
...
#
    ...
    def create(self, values):
        ...
        line = super(BKALine, self).create(values)
    ...
    def write(self, values):
        ...
        result = super(BKALine, self).write(values)
        return result


    order_id = fields.Many2one('account.bka', string='Order Reference', required=True, ondelete='cascade', index=True, copy=False)
...
#
    ...
    def name_get(self):
        if self._context.get('sale_show_order_product_name'):
            ...
            return result
        return super(BKALine, self).name_get()
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        if self._context.get('sale_show_order_product_name'):
            if operator in ('ilike', 'like', '=', '=like', '=ilike'):
                ...
                return self.search(domain, limit=limit).name_get()
        return super(BKALine, self).name_search(name, args, operator, limit)

    @api.multi
    def unlink(self):
        if self.filtered(lambda x: x.state in ('sale', 'done')):
            raise UserError(_('You can not remove a sales order line.\nDiscard changes and try setting the quantity to 0.'))
        return super(BKALine, self).unlink()
